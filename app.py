...
PRINTER_PORT    = 9100
SKU_FILE        = "/home/sysadmin/current_sku.txt"
STARTING_SKU    = 2000
LOG_FILE        = "/home/sysadmin/sku_log.txt"
CONFIG_FILE     = "/home/sysadmin/printer_config.json"
UPDATE_FILE     = "/home/sysadmin/available_updates.json"
SCRIPTS_FILE    = "/home/sysadmin/saved_scripts.json"
USERS_FILE      = "/home/sysadmin/app_users.json"
AUTH_CONFIG     = "/home/sysadmin/auth_config.json"
BRANDING_FILE   = "/home/sysadmin/branding.json"
UPDATE_INTERVAL = 6 * 60 * 60

app.secret_key = secrets.token_hex(32) if not os.path.exists("/home/sysadmin/.flask_secret") else open("/home/sysadmin/.flask_secret").read().strip()
if not os.path.exists("/home/sysadmin/.flask_secret"):
    with open("/home/sysadmin/.flask_secret","w") as f: f.write(app.secret_key)
...
def bridge_status():
    try:
        ...
        r = subprocess.run(["sudo","iptables","-t","nat","-L","POSTROUTING","-n","--line-numbers"],
                           capture_output=True,text=True)
        masq = "MASQUERADE" in r.stdout
        eth_ip = ""
        r2 = subprocess.run(["ip","addr","show","eth0"],capture_output=True,text=True)
        m = re.search(r'inet (\S+)',r2.stdout)
        if m: eth_ip = m.group(1)
        r3 = subprocess.run(["ip","addr","show","wlan0"],capture_output=True,text=True)
        m3 = re.search(r'inet (\d+\.\d+\.\d+\.\d+)',r3.stdout)
        wlan_ip = m3.group(1) if m3 else "--"
        return jsonify({"ok":True,"forwarding":fwd=="1","masquerade":masq,"eth_ip":eth_ip,"wlan_ip":wlan_ip})
    except Exception as e: return jsonify({"ok":False,"error":str(e)}), 500

@app.route("/api/bridge/apply", methods=["POST"])
def bridge_apply():
    d = request.json
    eth_ip      = d.get("eth_ip","192.168.50.1")
    eth_mask    = d.get("eth_mask","24")
    printer_ip  = d.get("printer_ip","192.168.50.2")
    try:
        cmds = [
            # Set static IP on eth0
            ["sudo","ip","addr","flush","dev","eth0"],
            ["sudo","ip","addr","add",f"{eth_ip}/{eth_mask}","dev","eth0"],
        ...
        rc_local = f"""#!/bin/bash
# Network bridge — auto-generated by SKU Print Manager
ip addr flush dev eth0
ip addr add {eth_ip}/{eth_mask} dev eth0
ip link set eth0 up
...
"""
        with open("/tmp/bridge_rc.sh","w") as f: f.write(rc_local)
        subprocess.run(["sudo","cp","/tmp/bridge_rc.sh","/etc/network/if-up.d/bridge"],capture_output=True)
        subprocess.run(["sudo","chmod","+x","/etc/network/if-up.d/bridge"],capture_output=True)
        # Also persist ip_forward
        subprocess.run(["sudo","sed","-i","s/#net.ipv4.ip_forward=1/net.ipv4.ip_forward=1/","/etc/sysctl.conf"],capture_output=True)
        return jsonify({"ok":True,"output":"\n\n".join(output),"eth_ip":eth_ip,"printer_ip":printer_ip})
    except Exception as e: return jsonify({"ok":False,"error":str(e)}), 500

@app.route("/api/bridge/remove", methods=["POST"])
def bridge_remove():
    try:
        subprocess.run(["sudo","iptables","-t","nat","-F","POSTROUTING"],capture_output=True)
        ...
        subprocess.run(["sudo","rm","-f","/etc/network/if-up.d/bridge"],capture_output=True)
        return jsonify({"ok":True})
    except Exception as e: return jsonify({"ok":False,"error":str(e)}), 500
...
# ── Branding ──────────────────────────────────────────────────────────────────
def load_branding():
    defaults = {"title": "SKU//PRINT", "subtitle": "SYSTEM MANAGEMENT"}
...
def save_branding(data):
    with open(BRANDING_FILE, "w") as f: json.dump(data, f, indent=2)
...
def set_branding():
    ...
    data = request.json
    b = load_branding()
    if "title"    in data: b["title"]    = data["title"].strip()[:40]
    if "subtitle" in data: b["subtitle"] = data["subtitle"].strip()[:60]
    save_branding(b)
    return jsonify({"ok": True, "branding": b})

# ── Hostname / NetBIOS ────────────────────────────────────────────────────────
@app.route("/api/system/hostname", methods=["POST"])
def set_hostname():
    if not require_auth("admin"): return jsonify({"error": "Unauthorized"}), 403
    name = request.json.get("hostname", "").strip()
    if not name or not re.match(r"^[a-zA-Z0-9\-]{1,63}$", name):
        return jsonify({"ok": False, "error": "Invalid hostname — use letters, numbers and hyphens only"}), 400
    try:
        output = []
        # Set hostname
        r1 = subprocess.run(["sudo", "hostnamectl", "set-hostname", name], capture_output=True, text=True)
        output.append(f"hostnamectl: {r1.stdout or r1.stderr or 'ok'}")
        # Update /etc/hosts
        r2 = subprocess.run(["sudo", "sed", "-i",
            f"s/127.0.1.1.*/127.0.1.1\t{name}/",
            "/etc/hosts"], capture_output=True, text=True)
        output.append(f"/etc/hosts: {'ok' if r2.returncode==0 else r2.stderr}")
        # Update NetBIOS via nmbd if available
        try:
            r3 = subprocess.run(["sudo", "sed", "-i",
                f"s/netbios name = .*/netbios name = {name.upper()}/",
                "/etc/samba/smb.conf"], capture_output=True, text=True)
            if r3.returncode == 0: output.append("samba netbios: ok")
        except: pass
        return jsonify({"ok": True, "hostname": name, "output": "\n".join(output)})
    except Exception as e:
